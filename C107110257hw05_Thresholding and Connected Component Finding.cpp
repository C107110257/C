// C107110257hw05.cpp : 此檔案包含 'main' 函式。程式會於該處開始執行及結束執行。
//
#include "myHist.h"
#include <algorithm>
#include <vector>
...
int main()
{
    cv::Mat m_in = cv::imread("license_plate.bmp", 0);
    int ROW = m_in.rows;
    int COL = m_in.cols;
    ...
    cv::Point origin;
    origin.x = COL / 2 - text_size.width / 2;
    origin.y = ROW / 2 + text_size.height / 2;
